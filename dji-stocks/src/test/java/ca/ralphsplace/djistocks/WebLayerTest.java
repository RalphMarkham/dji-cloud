...
@WebMvcTest(
    value = StockDataController.class,
    includeFilters = {@ComponentScan.Filter})
class WebLayerTest {

  StockDataRecord t1 =
      StockDataRecordUtil.buildTradeDataRecord(
          "1",
          "AA",
          "1/14/2011",
          "$16.71",
  ...
  @MockBean private StockDataService repository;

  @Test
  void shouldReturnSavedTradeData() throws Exception {
    when(repository.save(t1.toClientStockData("abc")))
        .thenReturn(CompletableFuture.completedFuture(t1));
  ...
  void shouldReturnQueriedTradeData() throws Exception {
    when(repository.findByStock("abc", "AA"))
        .thenReturn(CompletableFuture.completedFuture(Set.of(t1)));

    MvcResult mvcResult =
        mockMvc
            .perform(get("/api/stock-data/AA").header("X-Client_Id", "abc"))
            .andDo(print())
            .andExpect(request().asyncStarted())
            .andDo(MockMvcResultHandlers.log())
            .andReturn();
  ...
  @Test
  void shouldReturnBulkSavedTradeData() throws Exception {
    String s =
    ...
    var tdrList =
        new CsvToBeanBuilder<StockDataRecord>(new BufferedReader(new StringReader(s)))
            .withType(StockDataRecord.class).build().stream().collect(Collectors.toList());
    var ctdList =
        tdrList.stream().map(t -> t.toClientStockData("abc")).collect(Collectors.toList());
    ...
    MvcResult mvcResult =
        mockMvc
            .perform(
                multipart("/api/stock-data/bulk-insert")
                    .file(tradeData)
                    .header("X-Client_Id", "abc"))
            .andDo(print())
            .andExpect(request().asyncStarted())
  ...
  void testUnauthorized() throws Exception {
    mockMvc.perform(multipart("/api/stock-data/bulk-insert")).andExpect(status().isUnauthorized());

    mockMvc.perform(get("/api/stock-data/RST")).andExpect(status().isUnauthorized());
  }
}
