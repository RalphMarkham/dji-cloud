...
class WebLayerTest {
    ...
    void shouldReturnSavedTradeData() throws Exception {
        when(repository.save(t1.toClientStockData("abc"))).thenReturn(CompletableFuture.completedFuture(t1));
    ...
    void shouldReturnBulkSavedTradeData() throws Exception {
        String s = "quarter,stock,date,open,high,low,close,volume,percent_change_price,percent_change_volume_over_last_wk,previous_weeks_volume,next_weeks_open,next_weeks_close,percent_change_next_weeks_price,days_to_next_dividend,percent_return_next_dividend\n" +
        ...
        var ctdList = tdrList.stream().map(t -> t.toClientStockData("abc")).collect(Collectors.toList());
