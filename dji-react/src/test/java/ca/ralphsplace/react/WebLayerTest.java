...
class WebLayerTest extends UnitTestConstants {
  ...
  void postShouldReturnSavedStockData() {
    when(stockDataService.save(EXPECTED_SDR.toClientStockData(CLIENT_ID)))
        .thenReturn(Mono.just(EXPECTED_SDR));
    ...
    webClient
        ...
        .expectHeader()
        .contentType(APPLICATION_JSON)
        .expectBody(StockDataRecord.class)
        .isEqualTo(EXPECTED_SDR);
  }
  ...
  void getShouldReturnQueriedSingleTradeData() {
    ...
    webClient
        ...
        .expectHeader()
        .contentType(APPLICATION_JSON)
        .expectBodyList(StockDataRecord.class)
        .isEqualTo(List.of(EXPECTED_SDR));
  }

  @Test
  void postShouldReturnBulkSavedTradeData() {
    when(stockDataService.bulkSave(anyList())).thenReturn(Flux.fromIterable(EXPECTED_SDR_LIST));
    ...
    webClient
        .post()
        .uri("/api/stock-data/bulk-insert/")
        .header(ClientId.HEADER, CLIENT_ID)
        .accept(APPLICATION_JSON)
